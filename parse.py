...
def save_samples(data, prob_dir):
    ...
    for i, t in enumerate(data['tests'], start=1):
        with open(prob_dir / f'in{i}.in', 'w') as f:
            f.write(t['input'])
        with open(prob_dir / f'out{i}.out', 'w') as f:
            f.write(t['output'])
...
def make_prob(data, name=None):
    ...
    if name == '.':
        ...
        try:
            subprocess.check_call([MAKE_PROB, name], stdout=sys.stdout, stderr=sys.stderr, shell=True)
